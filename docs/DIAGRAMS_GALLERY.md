...
This document contains all visual blueprints, system flowcharts, and sequence diagrams created for **SecureRAG**.

---

## 🖼️ 1. Architecture Infographic Blueprint

![SecureRAG v3 Enterprise Architecture](assets/architecture_diagram.png)

---

## 🏗️ 2. System Architecture Topology Flowchart

> [!TIP]
> You can also open the standalone [architecture_topology.mmd](architecture_topology.mmd) file directly in the IDE to zoom, pan, and export.
...
## 📥 3. Document Ingestion & Dual-Indexing Workflow

> [!TIP]
> You can also open the standalone [ingestion_sequence.mmd](ingestion_sequence.mmd) file directly.
...
## 💬 4. Chat, Sub-10ms Semantic Cache & SSE Streaming Workflow

> [!TIP]
> You can also open the standalone [chat_stream_sequence.mmd](chat_stream_sequence.mmd) file directly.
...
sequenceDiagram
    ...
    alt Semantic Cache Hit (<10ms)
        ...
        ChatAPI->>LLM: Prompt LLM with reranked context & memory
...
## 📊 5. Key Architecture Specifications Summary Table

| Component | Technology | Role & Performance Metric |
|:---|:---|:---|
| **Web Frontend** | React 19 + TypeScript (Bun) | High-performance SPA with SSE streaming, bundled in **1.84s**. |
| **API Gateway** | FastAPI + SlowAPI | REST/SSE endpoints with JWT RBAC and rate limiting. |
